...
import { convertToISO8601 } from "./chatUtils.js";
...
export async function handleMessage(socketIO, bot, chatId, data) {
  ...
  if (match) {
    ...
    try {
      const savedMessage = await prisma.message.create({
        data: {
          text: clientCoinGeckoMessage.text,
          name: clientCoinGeckoMessage.name,
          userId: clientCoinGeckoMessage.userId,
          socketID: clientCoinGeckoMessage.socketID,
          datetime: clientCoinGeckoMessage.datetime,
          sentFrom: clientCoinGeckoMessage.sentFrom,
        },
      });
      console.log("Message coingecku price data saved in DB", savedMessage);
    } catch (error) {
      console.error("Error saving message:", error);
      throw error;
    }
...
export async function handleTelegramMessage(chatId, socketIO, data, bot) {
  ...
  try {
    const savedMessage = await prisma.message.create({
      data: {
        ...
        userId: telegramMessage.userId.toString(),
        socketID: telegramMessage.socketID,
        datetime: telegramMessage.datetime,
        sentFrom: telegramMessage.sentFrom,
      },
    });
    console.log("Message saved in DB", savedMessage);
  } catch (error) {
  ...
  if (match) {
    ...
    try {
      const savedMessage = await prisma.message.create({
        data: {
          text: telegramCoinGeckoMessage.text,
          name: telegramCoinGeckoMessage.name,
          userId: telegramCoinGeckoMessage.userId,
          socketID: telegramCoinGeckoMessage.socketID,
          datetime: telegramCoinGeckoMessage.datetime,
          sentFrom: telegramCoinGeckoMessage.sentFrom,
        },
      });
      console.log("Message coingecku price data saved in DB", savedMessage);
    } catch (error) {
